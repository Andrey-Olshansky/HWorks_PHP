...
implode() — склеивает массив в строку через разделитель
...
Ссылка (&) — позволяет изменять оригинальный массив, а не его копию.
С &:
Функция работает напрямую с оригинальным массивом
Изменения сохраняются после вызова*/
...
array_filter() — фильтрует массив, оставляя только элементы, 
подходящие под условие.
В твоём коде:
Оставляет в массиве $items только те элементы, которые НЕ равны 
$item (удаляет конкретный товар).*/
